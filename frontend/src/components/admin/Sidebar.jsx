...
const IcoAgents  = () => <UserCheck size={20} />;
const IcoClients = () => (
  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
const NAV_ITEMS = [
  ...
  { label: "Gestion des agents",       path: "/admin/agents",       icon: IcoAgents                },
...
export default function Sidebar({ collapsed = false, onToggle, isMobile = false, onClose }) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {!collapsed && (
          <span className="text-white/80 shrink-0">
            {isMobile ? <IcoClose /> : <IcoChevron collapsed={collapsed} />}
          </span>
        )}
      </div>

      {!collapsed && (
        <p className="text-[11px] font-bold text-orange-200 uppercase tracking-[0.18em] px-5 mb-2">MANAGE</p>
      )}
          ...
          return (
            <NavLink
              ...
              className={[
                ...
                active ? "bg-white text-[#FF6600] shadow-md" : "text-white hover:bg-white/20",
              ].join(" ")}
            >
              <span className={`shrink-0 ${active ? "text-[#FF6600]" : "text-white/90"}`}><Icon /></span>
      ...
      <div className="px-4 pb-6 mt-auto">
        ...
        {collapsed
          ? <div className="flex justify-center"><div className="w-2 h-2 rounded-full bg-orange-200/50" /></div>
          : <p className="text-[11px] text-orange-200/60 text-center">Johann Finance SA © 2026</p>
        }
      </div>
    </aside>
  );
