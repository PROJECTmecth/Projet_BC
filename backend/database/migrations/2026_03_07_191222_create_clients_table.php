<?php
use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;
use Illuminate\Support\Facades\DB;
return new class extends Migration {
    public function up(): void
    {
        Schema::create('clients', function (Blueprint $table) {
            $table->id('id_client');                     // Clé primaire auto-incrémentée
            $table->enum('genre',['Homme','Femme']);     //Genre du client
            $table->string('nom',100);                   //Nom de famille du client
            $table->string('prenom',100);                //Prénom du client
            $table->string('adresse',255);               // Adresse physique
            $table->string('ville',100);                 //Ville de résidence
            $table->string('activite',150);              //Activité professionnelle du client
            $table->enum('nationalite',['Résident','Étranger']);   //Statut de résidence
            $table->enum('type_piece',['CNI','NIU','Passeport','Permis']); //Type de pièce d'identité présentée
            $table->string('num_piece',50);                                //Numéro de la pièce d'identité     
            $table->string('telephone',20);                               //Numéro de téléphone du client
            $table->unsignedBigInteger('id_agent');                       //FK agent(Agent ayant enregistré le client)
            $table->dateTime('created_at')->default(DB::raw('CURRENT_TIMESTAMP')); //Date d'enregistrement dans le système

            $table->foreign('id_agent')->references('id_agent')->on('agents')->onDelete('cascade');  //FK agents
        });
    }
    public function down(): void
    {
        Schema::dropIfExists('clients');
    }
};